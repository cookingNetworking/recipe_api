...
class RecipeSerialzier(serializers.ModelSerializer):
    ...
    ingredients = CustomSlugRelatedField(
        many=True, slug_field="name", queryset=Ingredient.objects.all()
    )
    ...
    def _extract_steps_data(self, data):
        ...
        for i in range(len(steps_data)):
            steps_data[i]["photo"] = self.context["request"].FILES.get(
                f"steps.{i}.photo"
            )
        return steps_data
...
class RecipeCommentSerializer(serializers.ModelSerializer):
    ...
    recipe = CustomSlugRelatedField(
        many=False, slug_field="id", queryset=Recipe.objects.all()
    )
    ...
    def create(self, validated_data):
        ...
        if recipe:
            comment = RecipeComment.objects.create(
                user=req_user, recipe=recipe, **validated_data
            )
            return comment
        else:
...
class RecipeSQLDetailSerializer(RecipeSerialzier):
    ...
    class Meta(RecipeSerialzier.Meta):
        fields = RecipeSerialzier.Meta.fields + [
            "steps",
            "create_time",
            "likes",
            "save_count",
        ...
        read_only_fields = RecipeSerialzier.Meta.read_only_fields + [
            "likes",
            "save_count",
            "views",
            "top_five_comments",
...
class SaveActionSerializer(serializers.Serializer):
    ...
    recipe_id = serializers.IntegerField(
        help_text="The ID of the recipe to like.", required=False
    )
