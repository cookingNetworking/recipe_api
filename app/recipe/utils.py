...
def rsa_signer(message):
    ...
    private_key = serialization.load_pem_private_key(
        settings.AWS_CLOUDFRONT_KEY, password=None, backend=default_backend()
    )
...
def create_signed_url(image_path, expired_time):
    ...
    signed_url = cloudfront_signer.generate_presigned_url(
        url, date_less_than=expired_time
    )
    return signed_url
...
def saved_action(user, obj):
    ...
    if isinstance(obj, models.Recipe):
        ...
        if created:
            ...
            recipe_redis_handler.increase_recipe_view(
                hkey_name="save_count", recipe_id=obj.id
            )
            return Response(
                {"message": "User save the recipe!"}, status=status.HTTP_200_OK
            )
        elif save:
            obj.save_count = F("save_count") - 1
            obj.save(update_fields=["save_count"])
            save.delete()
            recipe_redis_handler.increase_recipe_view(
                hkey_name="save_count", recipe_id=obj.id, increment_value=-1
            )
            return Response(
                {"message": "User unsaved the recipe !"}, status=status.HTTP_200_OK
            )
        else:
            return Response(
                {"error": "Unknow error"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
        ...
        if created:
            ...
            obj.save(update_fields=["save_count"])
            return Response(
                {"message": "User save the tag!"}, status=status.HTTP_200_OK
            )
        elif save:
            obj.save_count = F("save_count") - 1
            obj.save(update_fields=["save_count"])
            save.delete()
            return Response(
                {"message": "User unsaved the tag !"}, status=status.HTTP_200_OK
            )
        else:
            return Response(
                {"error": "Unknow error"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        ...
        if created:
            ...
            obj.save(update_fields=["save_count"])
            return Response(
                {"message": "User save the ingredient!"}, status=status.HTTP_200_OK
            )
        elif save:
            obj.save_count = F("save_count") - 1
            obj.save(update_fields=["save_count"])
            save.delete()
            return Response(
                {"message": "User unsaved the ingredient !"}, status=status.HTTP_200_OK
            )
        else:
            return Response(
                {"error": "Unknow error"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        ...
        return Response(
            {"error": "No valid object found"}, status=status.HTTP_400_BAD_REQUEST
        )
