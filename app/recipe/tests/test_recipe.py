...
class PublicAgentAPITests(TestCase):
    ...
    def test_recipe_list_public(self):
        ...
        params = {"password": "test1123123123", "username": "testuser1"}
    ...
    def test_recipe_retrieve_public(self):
        ...
        with patch(
            "recipe.redis_set.RedisHandler.increase_recipe_view"
        ) as mock_increase_recipe_view, patch(
            "recipe.redis_set.RedisHandler.set_recipe"
        ) as mock_set_recipe:
            params = {"password": "test1123123123", "username": "testuser1"}
            user = create_user(**params)
    ...
    def test_recipe_list_filter_by_ingredient(self):
        ...
        ingredient_1 = Ingredient.objects.create(name="chocolate")
        recipe1.ingredients.add(ingredient_1)
        recipe2_params = {
            "title": "sampletitle1",
            "cost_time": "25",
            "description": "sample1",
        ...
        recipe2 = Recipe.objects.create(user=user, **recipe2_params)
        url = "/api/recipes/?ingredients=chocolate"
        res = self.client.get(url)
        res_content = decode_content(res.content)
    ...
    def test_get_comments_list_public(self):
        ...
        comment_1 = RecipeComment.objects.create(
            user=user, recipe=recipe, comment="test comment1", rating=5
        )
        comment_2 = RecipeComment.objects.create(
            user=user, recipe=recipe, comment="test comment2", rating=3
        )
        res = self.client.get(RECIPE_COMMENT_URL, {"page": 1, "recipe_id": recipe.id})
        res_content = decode_content(res.content)
        comment_ids = [recipe["comment_id"] for recipe in res_content["results"]]
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(len(res_content["results"]), 2)
    ...
    def test_ingredients_list_public(self):
        ...
        ingredient_1 = Ingredient.objects.create(name="chocolate")
        ingredient_2 = Ingredient.objects.create(name="banana")
        ingredient_3 = Ingredient.objects.create(name="beef")
...
class PrivateAgentAPITests(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        user_params = {"email": "test1@example.com", "username": "test1"}
    ...
    def test_retrieve_recipe(self):
        ...
        with patch(
            "recipe.redis_set.RedisHandler.set_recipe"
        ) as mock_set_recipe, patch(
            "recipe.redis_set.RedisHandler.increase_recipe_view"
        ) as mock_increase_recipe_view:
            mock_set_recipe.return_value = None
            mock_increase_recipe_view.return_value = None
            res = self.client.get(url)
            res_content = decode_content(res.content)
    ...
    def test_create_recipe(self):
        ...
        with patch(
            ...
            "recipe.redis_set.RedisHandler.set_hkey"
        ) as mock_update_hkey:
            mock_set_recipe.return_value = None
    ...
    def test_put_recipe(self):
        ...
        with patch(
            ...
            "recipe.redis_set.RedisHandler.update_hkey"
        ) as mock_update_hkey:
            mock_set_recipe.return_value = None
            mock_update_hkey.return_value = None
    ...
    def test_delete_recipe(self):
        ...
        recipe = create_recipe(user=self.user, **payload)
        url = detail_url(recipe.id)
        with patch("recipe.redis_set.RedisHandler.del_hkey") as mock_del_hkey, patch(
            "recipe.redis_set.RedisHandler.del_prev_hkey"
        ) as mock_del_prev_hkey:
    ...
    def test_add_comment(self):
        ...
        with patch(
            "recipe.redis_set.RedisHandler.update_recipe_in_cache"
        ) as mock_update_recipe_in_cache, patch(
            "recipe.redis_set.RedisHandler.set_recipe"
        ) as mock_set_recipe, patch(
            "recipe.redis_set.RedisHandler.increase_recipe_view"
        ) as mock_increase_recipe_view:
            ...
            self.assertEqual(
                res_content2["top_five_comments"][0]["comment_id"],
                res_content1["comment_id"],
            )
    ...
    def test_update_comment(self):
        ...
        recipe = create_recipe(user=self.user, **recipe_params)
        comment_params = {"comment": "for update test", "rating": 5}
        recipe_comment = RecipeComment.objects.create(
            user=self.user, recipe=recipe, **comment_params
        )
        with patch(
            "recipe.redis_set.RedisHandler.update_recipe_in_cache"
        ) as mock_update_recipe_in_cache:
            mock_update_recipe_in_cache.return_value = None
            payload = {"recipe": recipe.id, "comment": "updated!!!", "rating": 5}
            url = detail_comment_rul(recipe_comment.id)
            res = self.client.put(url, payload)
            self.assertEqual(res.status_code, status.HTTP_200_OK)
            res_content = decode_content(res.content)
            self.assertEqual(res_content["comment_id"], recipe_comment.id)
    ...
    def test_partial_update_comment(self):
        ...
        recipe = create_recipe(user=self.user, **recipe_params)
        comment_params = {"comment": "for partial_update test", "rating": 5}
        recipe_comment = RecipeComment.objects.create(
            user=self.user, recipe=recipe, **comment_params
        )
        with patch(
            ...
            mock_update_recipe_in_cache.return_value = None
            payload = {"recipe": recipe.id, "rating": 3}
            url = detail_comment_rul(recipe_comment.id)
    ...
    def test_destroy_comment(self):
        ...
        recipe = create_recipe(user=self.user, **recipe_params)
        comment_params = {"comment": "for destroy test", "rating": 5}
        recipe_comment = RecipeComment.objects.create(
            user=self.user, recipe=recipe, **comment_params
        )
        with patch(
    ...
    def test_clear_recipe_tags(self):
        ...
        with patch(
            ...
            mock_update_hkey.return_value = None
            res = self.client.patch(url, payload)
            print(decode_content(res.content))
            self.assertEqual(res.status_code, status.HTTP_200_OK)
            recipe.refresh_from_db()
    ...
    def test_save_ingredient(self):
        ...
        ingredient = Ingredient.objects.create(name="chocolate")
        payload = {"ingredient": f"{ingredient.name}"}
        with patch(
            "recipe.redis_set.RedisHandler.increase_recipe_view"
        ) as mock_increase_recipe_view:
            mock_increase_recipe_view.return_value = True
